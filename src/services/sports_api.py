...
# Cache for squads to avoid burning rate limits (squads rarely change daily)
squad_cache = {}

@rate_limit(calls_per_minute=8)
def fetch_team_squad(team_id: int) -> dict:
    """
    Fetch the factual real-world current squad and coach for a team to definitively prevent AI roster hallucinations.
    Uses a 24-hour TTL cache because squads do not change outside of transfer windows.
    """
    global squad_cache
    
    if team_id in squad_cache:
        cache_entry = squad_cache[team_id]
        if datetime.now() - cache_entry["fetched_at"] < timedelta(hours=24):
    ...
    try:
        print(f"🌐 Fetching official squad for Team ID {team_id}...")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        ...
        if 'squad' in data:
            for player in data['squad']:
                # The API usually provides 'name' and 'position'
                pos = player.get('position', '')
                # Filter out pure staff/management from playing roster if role is present
                if player.get('role') == 'PLAYER':
                    squad.append(f"{player.get('name')} ({pos})")
                elif pos: # Fallback if 'role' isn't explicitly PLAYER but position exists
                     squad.append(f"{player.get('name')} ({pos})")
        
        coach_name = data.get('coach', {}).get('name', 'Unknown')
        
        result = {"coach": coach_name, "roster": squad}
