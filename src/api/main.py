...
def predict_batch(request: MatchBatchRequest):
    ...
    for match_id in request.match_ids:
        ...
        # 8. Predict (Agent 1)
        initial_prediction = predict_match(home_team, away_team, stats, news, odds, h2h_data, home_form, away_form)
        
        # 8. Risk Manager Review (Agent 2)
        final_prediction = risk_manager_review(initial_prediction)
        
        # Inject logos
        final_prediction['home_logo'] = stats.get('homeTeam', {}).get('crest')
        final_prediction['away_logo'] = stats.get('awayTeam', {}).get('crest')
