...
def risk_manager_review(initial_prediction_json: dict, match_date: str = None) -> dict:
    ...
    - **THE CUMULATIVE FATIGUE OVERRIDE**: Use your Search Tool to verify if the underdog played an extra-time (120-minute) fixture in the last 7 days. If they did, you MUST REJECT or DOWNGRADE any 'Under' goals or 'Underdog Draw No Bet' picks. Physical exhaustion triggers a late-game collapse. Force Agent 1 to pivot to the favorite's offensive markets or 'NO BET'.
    ...
    *** AUDIT RULES ***
    ...
    - STRICT OUTPUT: The 'ai_recommended_bet' MUST be exactly ONE actionable bet. NEVER offer multiple choices or conversational text in this field.
      ...
      - **RULE 6: THE ANTI-BIAS PROTOCOL (CRITICAL)**:
        ...
        5. **THE CUMULATIVE FATIGUE OVERRIDE**: Use your Search Tool to verify if the underdog played an extra-time (120-minute) fixture in the last 7 days. If they did, you MUST REJECT or DOWNGRADE any 'Under' goals or 'Underdog Draw No Bet' picks. Physical exhaustion triggers a late-game collapse. Force Agent 1 to pivot to the favorite's offensive markets or 'NO BET'.
        6. **ESTIMATED ODDS**: You MUST provide a realistic `estimated_odds` (Decimal format) for your recommended bet. Use Agent 1's odds or the Odds API payload as a reference. If no odds are available, estimate based on the implied probability of your own tactical analysis.
