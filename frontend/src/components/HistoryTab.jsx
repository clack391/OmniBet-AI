...
const HistoryTab = ({ onSelectHistoryItem, isActive, latestPredictions }) => {
    ...
    const renderPickCard = (pick, tierKey) => {
        ...
        const borderClass = { amber: 'border-amber-500/20 hover:border-amber-500/40', blue: 'border-blue-500/20 hover:border-blue-500/40', rose: 'border-rose-500/20 hover:border-rose-500/40' }[accentColor];
        const tipAccent = { amber: 'text-amber-500 / text-amber-400', blue: 'text-blue-400 / text-blue-300', rose: 'text-rose-400 / text-rose-300' }[accentColor];
        const survivalColor = pick.survival_rate >= 85 ? 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' : pick.survival_rate >= 75 ? 'text-blue-300 border-blue-500/30 bg-blue-500/10' : 'text-rose-400 border-rose-500/30 bg-rose-500/10';

        return (
            <div key={pick.match_id || pick.teams} className={`bg-gray-900/80 backdrop-blur-md border ${borderClass} rounded-xl p-4 flex gap-4 transition-colors`}>
                <div className="flex-1 min-w-0">
                    {/* Date + Survival Rate */}
                    <div className="flex justify-between items-start mb-2">
                        <div className="text-xs text-gray-500 font-mono">
                            {(() => { try { if (!pick.match_date) return "Date Pending"; return new Date(pick.match_date).toLocaleString('en-GB', { timeZone: 'Africa/Lagos', dateStyle: 'short', timeStyle: 'short' }); } catch (e) { return pick.match_date || "Unknown Date"; } })()}
                        </div>
                        {pick.survival_rate != null && (
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono font-bold border ${survivalColor}`}>
                                ⚡ {pick.survival_rate}% survival
                            </span>
                        )}
                    </div>
                    ...
                    {/* Tip + Odds + Confidence */}
                    <div className={`inline-flex items-center px-3 py-1 rounded-full mb-3 flex-wrap gap-2 ${accentColor === 'amber' ? 'bg-amber-500/10 border border-amber-500/20' : accentColor === 'blue' ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-rose-500/10 border border-rose-500/20'}`}>
                        <div className="flex items-center">
                            <span className={`text-xs mr-2 font-bold ${accentColor === 'amber' ? 'text-amber-500' : accentColor === 'blue' ? 'text-blue-400' : 'text-rose-400'}`}>TIP:</span>
                            <span className={`text-sm font-black ${accentColor === 'amber' ? 'text-amber-400' : accentColor === 'blue' ? 'text-blue-300' : 'text-rose-300'}`}>{tipLabel}</span>
                        </div>
                        {pick.odds && <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-[10px] text-emerald-300 font-mono font-bold border border-emerald-500/30">{pick.odds}</span>}
                        {pick.confidence && <span className="px-2 py-0.5 rounded-full bg-black/50 text-[10px] text-gray-300 font-mono">{pick.confidence}%</span>}
                    </div>

                    {/* Portfolio Action */}
                    {pick.portfolio_action && (
                        <div className="text-[10px] text-gray-500 italic mb-2 leading-snug">{pick.portfolio_action}</div>
                    )}

                    {/* Math Reason */}
                    {pick.mathematical_reason && (
                        <div className="text-[10px] text-gray-400 bg-black/30 rounded px-2 py-1 mb-2 font-mono border-l-2 border-gray-600">{pick.mathematical_reason}</div>
                    )}
                    ...
                    <ul className="space-y-1">
                        {pick.reasoning && pick.reasoning.map((r, i) => (
                            <li key={i} className="text-xs text-gray-400 flex gap-2">
                                <span className={`mt-0.5 ${accentColor === 'amber' ? 'text-amber-500' : accentColor === 'blue' ? 'text-blue-400' : 'text-rose-400'}`}>•</span>
                                {r}
                            </li>
                        ))}
                    </ul>
    ...
    return (
        <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-6 w-full mt-6">
            ...
            {/* ── TIER 1: AI Master Accumulator ── */}
            {bestPicks && bestPicks.picks && bestPicks.picks.length > 0 && (
                <div className="mb-6 p-6 bg-gradient-to-br from-amber-500/10 to-orange-600/10 border border-amber-500/30 rounded-2xl relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-4 relative z-10">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-[10px] font-bold text-emerald-400 tracking-widest">≥85% SURVIVAL</span>
                            </div>
                            ...
                            <p className="text-amber-100/80 text-sm max-w-3xl leading-relaxed">{bestPicks.master_reasoning}</p>
                        </div>
                        <div className="flex gap-2 relative z-20">
                            <button onClick={() => handleAddAllToSlip(bestPicks.picks)} className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-black rounded-xl transition-all shadow-lg active:scale-95 text-xs uppercase tracking-tight border-b-4 border-amber-700 hover:border-amber-600">
                                <PlusCircle className="w-4 h-4" /> Add all to slip
                            </button>
                            <button onClick={handleClearBestPicks} className="p-2 text-amber-500/50 hover:text-amber-400 hover:bg-amber-500/10 rounded-lg transition-colors" title="Clear Best Picks">
                                <XCircle className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 relative z-10">
                        {bestPicks.picks.map((pick) => renderPickCard(pick, 'tier1'))}
                    </div>
                </div>
            )}

            {/* ── TIER 2: Value Trebles & Doubles ── */}
            {bestPicks && bestPicks.tier_2_picks && bestPicks.tier_2_picks.length > 0 && (
                <div className="mb-6 p-6 bg-gradient-to-br from-blue-500/10 to-indigo-600/10 border border-blue-500/30 rounded-2xl relative overflow-hidden">
                    <BarChart2 className="absolute -right-10 -bottom-10 w-64 h-64 text-blue-500/5 rotate-12 pointer-events-none" />
                    <div className="flex justify-between items-start mb-4 relative z-10">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <BarChart2 className="w-6 h-6 text-blue-400" />
                                <h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-indigo-400">
                                    TIER 2 — Value Trebles &amp; Doubles
                                </h3>
            ...
            {/* ── TIER 3: Singles & EV Snipes ── */}
            {bestPicks && bestPicks.tier_3_picks && bestPicks.tier_3_picks.length > 0 && (
                <div className="mb-6 p-6 bg-gradient-to-br from-rose-500/10 to-red-700/10 border border-rose-500/30 rounded-2xl relative overflow-hidden">
                    <XCircle className="absolute -right-10 -bottom-10 w-64 h-64 text-rose-500/5 rotate-12 pointer-events-none" />
                    <div className="flex justify-between items-start mb-4 relative z-10">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <XCircle className="w-6 h-6 text-rose-400" />
                                <h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-300 to-red-400">
                                    TIER 3 — Singles &amp; EV Snipes
                                </h3>
